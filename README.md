...
| **"Called in wrong state: STATE_INPROGRESS"** | A description applied twice on a pre-promise engine. Fixed in v1.0.8; if you still see it the TV is serving a cached page — clear its cache or add `?x=1`. |
| **"createAnswer... 1 argument required, but only 0 present"** | A WebRTC stack older than promises (Chromium 49 and similar). Handled automatically since v1.0.7 by falling back to the callback API; if you see it, the TV is serving a cached copy — clear its cache or add `?x=1`. |
...
| **Black screen, no error at all** | Press 🔴 Red. `no media arriving` means transport — firewall, or tick *Strip .local candidates*. `arriving but no frames render` means the TV cannot decode what is being sent. Anything else means it is playing and the problem is elsewhere. |
